...
public class Center extends JPanel{
	...
	public int color=1; //当前应该下的颜色
	public LinkedList<Chess> chesses=new LinkedList<Chess>(); //保存棋子
	public Center(GoBangFrame frame){
		this.frame=frame;
	...
	private void initSend() {
		...
		textarea1.setBorder(BorderFactory.createTitledBorder(
				...
				"发送",TitledBorder.LEFT,TitledBorder.TOP,
	...
	private void addListener(){
		button1.addActionListener(new ActionListener() { //发送按钮
			@Override
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					frame.out.println("Server:"+textarea1.getText());
					textarea2.setForeground(new Color(0,0,0));
					textarea2.append("Server:"+textarea1.getText());
					textarea2.append("\n");
	...
	private void addListener2(){
		button2.addActionListener(new ActionListener() { //复盘按钮
			@Override
			public void actionPerformed(ActionEvent e) {
				if (e.getSource() == null){
					return;
				...
				for(int i=0;i<=row;i++){
					for(int j=0;j<=col;j++){
						...
						if(pointer.isPoint(x, y)&&pointer.getTag()==0){
							...
							if(frame.getGameMode()==1){ //单机
								clearAILast();//清楚给电脑做的标记
								repaint();
	...
	public void addChess(int x,int y,int color){//将对方的棋子添加进来
		Chess chess1=new Chess(x,y,color);
		chesses.add(chess1);
		int i=(x-170)/64;
		int j=(y-45)/64;
		pointers[j][i].setTag(2);
		repaint();
	}
	public void chessWin(){
		gameFlag=false; //赢了之后不能再下棋
		repaint();
	}
