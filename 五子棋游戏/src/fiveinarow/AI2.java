...
//自定义排序类
class DataCount2 implements Comparator<Data2>{
...
public class AI2 {
		//电脑走一步
		public static boolean next(Center2 center){
			boolean flag=goWeight(center)||randomAI(center); //先利用权重分计算，当得不到满意的分数时，执行随机落子
			return flag;
		}
		
		//根据权重分落子
		public static boolean goWeight(Center2 center){
			LinkedList<Data2> datas=new LinkedList<Data2>();//存放数据
			...
			//对获取的权重分进行从大到小排序
			Collections.sort(datas,new DataCount2());
			for(int i=0;i<datas.size();i++){
				System.out.println("权重分:"+datas.get(i).getCount());
			}
			//取最大的作为落子的地方
			if(datas.size()>0){
				Data2 data2=datas.get(0);//取第一个元素作为下一次落子的位置
		...
		private static Data2 getData(int dir, int type, Center2 center,
				Pointer pointer) {
			Data2 resData=new Data2();//返回结果
			int i=pointer.getI();
			int j=pointer.getJ();
			Pointer[][] pointers=center.pointers;
			Pointer tempPointer;
		...
		private static void setCount(Data2 resData, int i, int j, int dir, int type, int num,
		...
		//随机落子
		public static boolean randomAI(Center2 center){
			//随机获取指示器位置
		...
		//随机获得指示器的位置
		private static Pointer getRandomPointer(Center2 center) {
		...
		//根据位置执行落子
		private static void chessHere(Pointer pointer, int color, Center2 center) {
